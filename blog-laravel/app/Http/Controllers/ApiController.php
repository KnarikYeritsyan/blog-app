...
//        $footer_links = MenuItem::where('menu_id',3)->whereNull('parent_id')->orderBy('sort_order')->get();
        ...
        return response()->json(compact('social_items'));
    }
    ...
    function get_product(Request $request)
    {
        ...
        $prev = Product::where('id', '<', $product->id)->max('id');
        $previous = Product::where('id', $prev)->first();
        $nextn = Product::where('id', '>', $product->id)->min('id');
        $next = Product::where('id', $nextn)->first();
        return response()->json(compact('product','previous','next'));
    }
    ...
    function get_videos()
    {
        $videos = Video::where('video_group','video')->where('status','1')->get();
        return response()->json(compact('videos'));
    }

    function get_webinar()
    {
        $webinar = Video::where('video_group','webinar')->first();
        return response()->json(compact('webinar'));
    }
    ...
    {
        $products = Product::where('status',1)->with('tag','category');
