...
//            'brand_id'=>isset($request['brand_id'])?$request['brand_id']:null,
    ...
    function post_create_video(Request $request)
    {
        $request->validate([
...
//            'content_am' => 'required',
        ...
        return redirect()->route('admin.video',['locale'=>app()->getLocale(),'id'=>$video->id])->with('message', __('Video created successfully'));
    }

    function edit_video(Request $request)
    {
        $request->validate([
            'slug' => 'required|unique:videos,slug,'.$request->upd_id,
            'title_am' => 'required',
//            'content_am' => 'required',
            'link' => 'required',
            'url_type' => 'required',
        ...
        $video = Video::where('id',$request->upd_id)->update([
            'title' => [
                'ru' => $request['title_ru'],
                'en' => $request['title_en'],
                'am' => $request['title_am'],
        ...
        return redirect()->back()->with('message', __('Video updated successfully'));
    }

    function edit_webinar(Request $request)
    {
        $request->validate([
            'slug' => 'required|unique:videos,slug,'.$request->upd_id,
            'title_am' => 'required',
//            'content_am' => 'required',
        ...
        Video::where('id',$request->upd_id)->update([
            'title' => [
                'ru' => $request['title_ru'],
                'en' => $request['title_en'],
                'am' => $request['title_am'],
        ...
        return redirect()->back()->with('message', __('Webinar updated successfully'));
    }

    function video_delete(Request $request)
    {
        Video::destroy($request->id);
        return redirect()->back()->with('message', __('Video deleted successfully'));
    }

    function slider_create_post(Request $request)
    {
        $request->validate([
//            'slug' => 'unique:sliders,slug',
//            'title_am' => 'required',
            'status' => 'required|in:0,1',
//            'description_am' => 'required',
        ]);
    ...
    {
        $request->validate([
//            'slug' => 'unique:sliders,slug,'.$request->upd_id,
            'status' => 'required|in:0,1',
//            'title_am' => 'required',
//            'description_am' => 'required',
        ]);
