...
//        $categories = Category::where('name->en', 'Perfumes')->get();
    ...
    {
        $videos = Video::where('video_group','video')->orderBy('created_at', 'desc');
        if (isset($request->search_term)){
    ...
    function webinar(Request $request)
    {
        $webinar = Video::where('video_group','webinar')->first();
        return view('admin.webinar.webinar',compact('webinar'));
    }

    function video_create()
    {
        return view('admin.video.video-new');
    }

    function show_video(Request $request)
    {
