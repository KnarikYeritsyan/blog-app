...
@section('breadcrumb')
    <ol class="breadcrumb mb-2">
        ...
        <li class="breadcrumb-item">{{__('Webinar')}}</li>
    </ol>
@endsection
@section('css')
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    @endif
                        ...
                        @endif
                    <h4 class="card-title mb-3">{{$webinar->title}}</h4>
                    <form method="POST" action="" enctype="multipart/form-data">
                        @csrf
                        <input hidden id="url_type" name="url_type" value="{{$webinar->url_type}}">
                        <input hidden id="url_link" name="link" value="{{$webinar->link}}">
                        <div class="form-group row">
                            <label for="video-link" class="col-sm-2 col-form-label">{{__('Video Link')}}</label>
                            <div class="col-sm-10">
                                <input id="video-link" name="name" type="text" class="form-control" value="{{$webinar->link}}" required>
                            </div>
                        </div>
                        <div class="form-group row">
                            <div class="col-sm-10" id="video-frame">
                                <div id="fb_vid" style="{{$webinar->url_type == 'iframe'?'display: none':''}}" class="fb-video" data-href="{{$webinar->link}}" data-show-text="false" data-width="auto"></div>
                                <iframe id="you_vid" style="{{$webinar->url_type == 'facebook'?'display: none':''}}" width="727" height="409" src="{{$webinar->link}}" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"  allowfullscreen></iframe>
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="slug" class="col-sm-2 col-form-label">{{__('Slug')}}</label>
                            <div class="col-sm-10">
                                <input id="slug" name="slug" type="text" class="form-control" value="{{$webinar->slug}}">
                            </div>
                        </div>
                        <div class="form-group row">
                            ...
                            <div class="col-sm-10">
                                <select name="status" id="status" class="form-control">
                                    <option <?php if($webinar->status) echo 'selected' ?> value="1">{{__('Published')}}</option>
                                    <option <?php if(!$webinar->status) echo 'selected' ?> value="0">{{__('Not Published')}}</option>
                                </select>
                            </div>
                        </div>
                        <input type="hidden" name="upd_id" value="{{$webinar->id}}">
                        ...
                        <div class="tab-content">
                            <div class="tab-pane" id="home-b2">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input id="title" name="title_ru" type="text" class="form-control" value="{{isset($webinar->translations['title']['ru'])?$webinar->translations['title']['ru']:''}}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="content" class="col-sm-2 col-form-label">{{__('Content')}}</label>
                                    <div class="col-sm-10">
                                        <textarea id="content" name="content_ru" class="summernote">{{isset($webinar->translations['content']['ru'])?$webinar->translations['content']['ru']:''}}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="tab-pane show" id="profile-b2">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input id="title_en" name="title_en" type="text" class="form-control" value="{{isset($webinar->translations['title']['en'])?$webinar->translations['title']['en']:''}}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="content" class="col-sm-2 col-form-label">{{__('Content')}}</label>
                                    <div class="col-sm-10">
                                        <textarea id="content" name="content_en" class="summernote">{{isset($webinar->translations['content']['en'])?$webinar->translations['content']['en']:''}}</textarea>
                                    </div>
                                </div>
                            </div>
                            <div class="tab-pane active" id="settings-b2">
                                <div class="form-group row">
                                    ...
                                    <div class="col-sm-10">
                                        <input id="title_am" name="title_am" type="text" class="form-control" value="{{isset($webinar->translations['title']['am'])?$webinar->translations['title']['am']:''}}">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label for="content" class="col-sm-2 col-form-label">{{__('Content')}}</label>
                                    <div class="col-sm-10">
                                        <textarea id="content" name="content_am" class="summernote">{{isset($webinar->translations['content']['am'])?$webinar->translations['content']['am']:''}}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
